...
class ChanceNode(Node):
    ...
    def get_health_utility(self):
        exp_health_utility = self.health_utility
        i = 0
        for node in self.futureNodes:
...
class DecisionNode(Node):
    ...
    def get_health_utility(self):
        """ returns the expected costs of future nodes"""
        outcomes_1 = dict() # dictionary to store the expected cost of future nodes along with their names as keys
        for node in self.futureNode:
            outcomes_1[node.name] = node.get_health_utility()
